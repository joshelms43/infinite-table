...
/* ---- spin: engine r4, the honest ball ----
   Every ball carries roll state beside velocity; draw and follow are backspin and
   topspin grabbing the cloth after contact. The claims: zero spin equals absent
   spin; close-range draw screws the cue well back behind the contact; a soft
   follow runs it well through; the object ball leaves on the same line either way
   (it may leave slower off a drawn cue — that is the cloth's honest tax on the
   approach); english bends cushion rebounds; and every spun shot is deterministic
   and comes to rest. */
{
  const at = (d) => [
    { id: 0, x: 1.1 - d, y: PP.H / 2, vx: 0, vy: 0, pocketed: false },
    { id: 1, x: 1.1, y: PP.H / 2, vx: 0, vy: 0, pocketed: false },
  ];
  const contact = 1.1 - 2 * PP.R;
  const plainA = PP.simulate(at(0.35), { angle: 0, power: 0.5 });
  const zeroed = PP.simulate(at(0.35), { angle: 0, power: 0.5, sx: 0, sy: 0 });
  T('spin — zero spin is identical to no spin', JSON.stringify(plainA.balls) === JSON.stringify(zeroed.balls));
  const draw = PP.simulate(at(0.35), { angle: 0, power: 0.5, sy: -1 });
  T('spin — a close power draw screws the cue well back', draw.balls[0].x < contact - 0.3,
    'back ' + (contact - draw.balls[0].x).toFixed(2) + 'm');
  const plainB = PP.simulate(at(0.5), { angle: 0, power: 0.35 });
  const follow = PP.simulate(at(0.5), { angle: 0, power: 0.35, sy: 1 });
  T('spin — a soft follow runs the cue well through', follow.balls[0].x > contact + 0.3,
    'through ' + (follow.balls[0].x - contact).toFixed(2) + 'm');
  T('spin — the object ball leaves on the same line regardless',
    Math.abs(draw.balls[1].y - plainA.balls[1].y) < 0.01 && Math.abs(follow.balls[1].y - plainB.balls[1].y) < 0.01);
  T('spin — a drawn cue arrives softer, and the object honestly shows it',
    draw.balls[1].x <= plainA.balls[1].x + 0.01);
  ...
  const a1 = PP.simulate(at(0.5), { angle: 0.02, power: 0.5, sx: 0.7, sy: -0.8 });
  const a2 = PP.simulate(at(0.5), { angle: 0.02, power: 0.5, sx: 0.7, sy: -0.8 });
  T('spin — a spun shot replays byte-identically', JSON.stringify(a1.balls) === JSON.stringify(a2.balls) && a1.ticks === a2.ticks);
  T('spin — and always comes to rest on its own', a1.ticks < PP.MAXTICKS && draw.ticks < PP.MAXTICKS && follow.ticks < PP.MAXTICKS);
}
